...
const CONTENT_DIR = path.join(process.cwd(), 'content', 'words');
const IMAGES_DIR = path.join(process.cwd(), 'public', 'notion-images', 'words');
 ...
 */
function needsUpdate(post: WordMetadata): boolean {
...
export async function toLocalMarkdown(posts: WordMetadata[]) {
  ...
  for (const post of posts) {
    try {
      ...
      if (!needsUpdate(post)) {
        skipped++;
        console.log(`⊘ Skipped (up-to-date): ${post.page_id}`);
        continue;
      }
